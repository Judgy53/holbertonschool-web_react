...
describe('<Notifications />', () => {
  ...
  describe('displayDrawer=true', () => {
    ...
    describe('listNotifications = [...]', () => {
      const listNotifications = new Map({
  ...
  it('does rerender when updating the props with a bigger list', () => {
    ...
    wrapper = shallow(<Notifications displayDrawer={true} listNotifications={new Map(listNotifications)} />);
    const renderMock = jest.spyOn(wrapper.instance(), 'render');

    wrapper.setProps({ listNotifications: new Map(biggerList) });
  ...
  describe('notification filter buttons', () => {
    ...
    beforeEach(() => {
      props.setNotificationFilter.mockClear();
      wrapper = shallow(<Notifications {...props} />);
    });


    it('calls setNotificationFilter with "urgent" when clicking the first filter button', () => {
      const filterUrgentButton = wrapper.find('[aria-label="Filter Urgent"]').at(0);
      filterUrgentButton.simulate('click');
      expect(props.setNotificationFilter).toHaveBeenCalledWith(NotificationTypeFilters.URGENT);
    });

    it('calls setNotificationFilter with "default" when clicking the second filter button', () => {
      const filterDefaultButton = wrapper.find('[aria-label="Filter Default"]').at(0);
      filterDefaultButton.simulate('click');
      expect(props.setNotificationFilter).toHaveBeenCalledWith(NotificationTypeFilters.DEFAULT);
    });
  })
});
