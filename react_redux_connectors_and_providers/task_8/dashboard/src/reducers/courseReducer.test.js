...
describe('uiReducer()', () => {
  ...
  it('returns the action data when action `FETCH_COURSE_SUCCESS` is passed', () => {
    ...
    const expected = new Map({
      courses: new Map(fetch_data.map(course => [String(course.id), { ...course, isSelected: false }]))
    });
    expect(courseReducer(initialState, { type: FETCH_COURSE_SUCCESS, courses: fetch_data })).toEqual(expected);
  });
  ...
  it('returns the correct data when action `SELECT_COURSE` is passed', () => {
    ...
    const expected = {
      courses: state.courses.setIn(['2', 'isSelected'], true)
    }
  ...
  it('returns the correct data when action `UNSELECT_COURSE` is passed', () => {
    ...
    const expected = {
      courses: state.courses.setIn(['2', 'isSelected'], false)
    }
